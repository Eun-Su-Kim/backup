...
#이 코드는 간단한 선형 회귀 문제이며 최적화 기능은 미니 배치 SGD입니다
...
def shuffle_data(x,y):
    ...
    #x, y 데이터를 임의로 방해하고 x와 y를 일대일로 유지
    seed = random.random()
    random.seed(seed)
    random.shuffle(x)
    random.seed(seed)
...
#모든 손실을 바꾸는 것이 중요합니다. 그 이유는 행렬이 왼쪽 위 모서리에서 오른쪽 아래 모서리까지 순서대로 요소를 정렬하고 
# 도면에서 왼쪽 아래 모서리를 원점으로 사용하기 때문입니다.
# 初始化a,b值 a, b 값을 초기화
a = 10.0
b = -20.0
...
for step in range(1,100):
    ...
    # 绘制图1中的loss点 그림 1에서 손실 지점을 그립니다.
